...
public:
    bool dfs(int u, int parent, vector<bool>& vis, vector<int> adj[]){
        vis[u] = 1;
        for(auto& v : adj[u]){
            if(!vis[v]){
                if(dfs(v, u, vis, adj)) {
    ...
    bool isCyclic(int n, vector<int> adj[]){
        ...
        {
            if(!vis[u]){
                if(dfs(u, -1, vis, adj)){ // first node doesn't have a parent node. so, -1
                    return 1;
                }
            }
        }
    ...
    bool bfs(int i, vector<bool>& vis, vector<int> adj[]){
        ...
        while(q.size()){
            ...
            for(auto& v : adj[u]){
                ...
                else if(v != parent){ // v is marked visited by some other way. this means that a cycle is formed
                    return 1;
                }
            }
        }
